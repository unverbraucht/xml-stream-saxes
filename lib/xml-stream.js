...
function parse() {
  ...
  xml.on('opentag', function(saxesTag) {
    name = saxesTag.name;
    attr = saxesTag.attributes;
  ...
  xml.on('closetag', function(saxesTag) {
    var name = saxesTag.name;
  ...
  var parseChunk = function(data) {
    if (self._encoding) {
      data = iconv.decode(data, self._encoding);
    }
    if (!xml.write(data)) {
